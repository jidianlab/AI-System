...
* [7.4 面向深度学习的集群管理系统](7.4-面向深度学习的集群管理系统.md#74-面向深度学习的集群管理系统)
  * [7.4.1 深度学习工作负载的需求](7.4-面向深度学习的集群管理系统.md#741-深度学习工作负载的需求)
    * [思考题](7.4-面向深度学习的集群管理系统.md#思考题)
  * [7.4.2 异构硬件的多样性](7.4-面向深度学习的集群管理系统.md#742-异构硬件的多样性)
  * [7.4.3 深度学习平台的管理与运维需求](7.4-面向深度学习的集群管理系统.md#743-深度学习平台的管理与运维需求)
    * [思考与实验设计](7.4-面向深度学习的集群管理系统.md#思考与实验设计)
  * [7.4.4 深度学习负载与异构硬件下的调度设计](7.4-面向深度学习的集群管理系统.md#744-深度学习负载与异构硬件下的调度设计)
  * [7.4.5 开源和云异构集群管理系统简介](7.4-面向深度学习的集群管理系统.md#745-开源和云异构集群管理系统简介)
  * [7.4.6 部署异构资源集群管理系统实验](7.4-面向深度学习的集群管理系统.md#746-部署异构资源集群管理系统实验)
    * [7.4.6.1 实验目的](7.4-面向深度学习的集群管理系统.md#7461-实验目的)
    * [7.4.6.2 实验环境](7.4-面向深度学习的集群管理系统.md#7462-实验环境)
    * [7.4.6.3 实验原理](7.4-面向深度学习的集群管理系统.md#7463-实验原理)
    * [7.4.6.4 实验内容](7.4-面向深度学习的集群管理系统.md#7464-实验内容)
    * [7.4.6.5 实验报告](7.4-面向深度学习的集群管理系统.md#7465-实验报告)
  * [小结与讨论](7.4-面向深度学习的集群管理系统.md#小结与讨论)
  * [参考文献](7.4-面向深度学习的集群管理系统.md#参考文献)

## 7.4.1 深度学习工作负载的需求

已有的深度学习作业调度工作（例如，[Gandiva OSDI '18](https://dl.acm.org/doi/10.5555/3291168.3291212) [\[2\]](7.4-面向深度学习的集群管理系统.md#gandiva)）对当前深度学习作业负载的特点进行了分析与总结。我们使用的集群管理系统管理的大多数作业是深度学习的训练作业而不是推理作业。深度学习训练作业相比传统的数据中心批处理作业（例如，大数据处理和分析作业）有一些新的特点和不同：\
“
...
* 稳定性
  ...
  * 由于节点上的异构硬件也有一定概率产生硬件问题（例如，[GPU 故障（Failure）](https://ieeexplore.ieee.org/abstract/document/7056044)）[\[1\]](7.4-面向深度学习的集群管理系统.md#failure)，造成平台稳定性的挑战。如何高效，敏捷的发现和修复故障，除了工具的支持，还需要系统化的系统设计，开发流程设计与管理策略设计共同作用。
* 可维护性
...
接下来，我们首先将从深度学习平台的调度算法入手，将介绍考虑不同设计目标和侧重点的调度算法设计。这些调度器由于设计目标不同，且基于能获取信息的 _**假设**_ 也不同，同时实现和对作业的入侵性也不同，读者在选用和设计调度器时，需要考虑不同算法的优劣势并根据平台现状酌情选择。
...
[Gandiva OSDI '18](https://dl.acm.org/doi/10.5555/3291168.3291212) [\[2\]](7.4-面向深度学习的集群管理系统.md#gandiva)根据深度学习作业特点（早反馈，分布式作业对拓扑敏感等），以及硬件和操作系统对 GPU 计算的进程上下文切换支持的不足进行弥补，通过框架和平台协同设计进而提升整体集群资源利用率。但是其假设是框架提供支持基本检查点和迁移的功能原语，所以具体实现需要入侵性的进行框架的修改。

Gandiva 设计了两种模式：

一种是 _**反应模式（Reactive Mode）**_ ：

类似传统调度器事件驱动的设计，根据不同事件和状态（作业到达（Arrivals）, 离开（Departures）, 失效（Failures））触发调度策略，可以抽象理解其整体策略为一个状态机（State Machine）。

![](img/4/affinity.png)图 7.4.1 分布式作业调度受局部性影响 ([图片引用 HiveD OSDI '20](https://www.usenix.org/sites/default/files/conference/protected-files/osdi20_slides_zhao.pdf))
...
![](img/4/7-4-6-gandivaplacement.png)图 7.4.2 16 块 GPU 集群中，Gandiva 调度实例 ([图片引用 Gandiva OSDI '18](https://www.usenix.org/system/files/osdi18-xiao.pdf))

另一种是 _**内省模式（Introspective Mode）**_ ：
...
![](img/4/7-4-7-jobmitivationgandiva.png)图 7.4.3 共享资源集群中，Gandiva 进行作业迁移实例 ([图片引用 Gandiva OSDI '18](https://www.usenix.org/system/files/osdi18-xiao.pdf))

Gandiva[\[2\]](7.4-面向深度学习的集群管理系统.md#gandiva) 的设计方法能大幅度降低作业执行延迟和提升资源利用率，但需要结合其他调度策略才能兼顾公平性等目标，同时需要框架和平台提供功能支持。同时也存在很多其他挑战，例如：需要能够监控深度学习作业负载信息，同时越来越多样的资源使用状况（例如，动态性的模型训练）的深度学习作业会产生新的挑战。

Gandiva[\[2\]](7.4-面向深度学习的集群管理系统.md#gandiva) 本身需要框架或硬件提供检查点（Checkpoint）和实时迁移 (Live Migrtation)的支持，这点假设对现有平台挑战较大，因为框架与平台本身是分离的软件。需要框架和平台协同设计，对一些公司或平台两者为分离团队，协同开发和维护也有新的挑战。

利用运行时信息反馈和框架与平台协同设计的调度器工作还有：

* 基于在线（Online）作业信息反馈，进行调度算法优化设计的调度器 [Optimus EuroSys '18](https://dl.acm.org/doi/10.1145/3190508.3190517)[\[3\]](7.4-面向深度学习的集群管理系统.md#optimus) 等。
* 需要框架和平台协同设计提供支持的调度器还有 [AntMan OSDI '20](https://dl.acm.org/doi/abs/10.5555/3488766.3488796)[\[4\]](7.4-面向深度学习的集群管理系统.md#antman) 等。
...
表 7.4.1 Tiresias 对深度学习调度器比较总结 ([表格引用 Tiresias NSDI '19](https://www.usenix.org/system/files/nsdi19-gu.pdf))
...
![](img/4/queuedelay.png)图 7.4.4 排队延迟问题 ([图片引用 OSDI '20](https://www.usenix.org/sites/default/files/conference/protected-files/osdi20_slides_zhao.pdf))
...
![](img/4/7-4-5-hivedmotivatingexample.png)图 7.4.5 HiveD机架的多级单元分配示例。 ([图片引用 HiveD OSDI '20](https://www.usenix.org/system/files/osdi20-zhao_hanyu.pdf))
...
![](img/4/pai.png)图 7.4.6 OpenPAI架构图 ([图片引用 OpenPAI 文档](https://github.com/microsoft/pai))
...
Philly 是微软内部使用的大规模深度学习训练作业平台，Philly 旨在支持执行有监督式机器学习的训练工作负载。这包括培训来自开发产品的生产团队的工作，这些产品使用用于图像分类、语音识别等的模型。有相关研究工作对 Philly 上的[资源分配，深度学习作业性能特点](https://dl.acm.org/doi/10.5555/3358807.3358888)[\[5\]](7.4-面向深度学习的集群管理系统.md#yarncs)和[深度学习程序缺陷](https://dl.acm.org/doi/10.1145/3377811.3380362)[\[8\]](7.4-面向深度学习的集群管理系统.md#programfail)进行研究，从中我们可以观察和了解大规模生产环境中作业资源争用，资源利用率，调度策略设计和程序缺陷等问题及启发相关新的研究工作。

* 公有云面向第三方（Third-party）人工智能平台服务-[Singularity](https://arxiv.org/abs/2202.07848)[\[9\]](7.4-面向深度学习的集群管理系统.md#singularity)
...
1.  深度学习集群管理系统
    ...
    _**主节点**_ 管理集群中的工作节点和 Pod，通常运行控制组件，对集群做出全局决策（比如调度），以及检测和响应集群事件，主要包括以下组件（更详细的功能介绍请参考[官方文档](https://kubernetes.io/zh/docs/concepts/overview/components/)）：
    ...
    *   cloud-controller-manager
    ...
    _**工作节点**_ 托管作为应用程序组件的 Pod，维护运行的 Pod 并提供 Kubernetes 运行环境，主要包括以下组件：
...
![](img/4/7-4-4-components-of-kubernetes.svg)图 7.4.7 Kuberenetes控制平面组件（Control Plane Components）\
([图片引用 Kubernetes 文档](https://kubernetes.io/zh/docs/concepts/overview/components))
...
![](img/6/Lab6-flow.png)图 7.4.8 实验流程图([图片来源](7.4-面向深度学习的集群管理系统.md))
